...
n_replicates <- env_integer("SPATIALESS_STRATIFIED_REPLICATES", 200L)
nperm <- env_integer("SPATIALESS_STRATIFIED_NPERM", 999L)
seed <- env_integer("SPATIALESS_STRATIFIED_SEED", 20260731L)
grid_side <- env_integer("SPATIALESS_STRATIFIED_GRID_SIDE", 20L)
block_side <- env_integer("SPATIALESS_STRATIFIED_BLOCK_SIDE", 5L)
if (grid_side %% 2L != 0L || grid_side < 10L ||
    grid_side %% block_side != 0L || block_side < 2L) {
  stop("Grid side must be even and divisible by a block side of at least two.",
       call. = FALSE)
}
out_dir <- Sys.getenv(
  "SPATIALESS_STRATIFIED_OUT",
  file.path(
    project_dir, "benchmarks", "results",
    sprintf("h1_stratified_analytic_rep%d_perm%d", n_replicates, nperm)
  )
)
...
simulate_expression <- function(replicate_seed, effect = 1) {
  ...
  shared_block <- stats::rgamma(
    length(blocks$block_levels), shape = 4, rate = 4
  )
  ligand_block <- shared_block * stats::rlnorm(
    length(shared_block), meanlog = 0, sdlog = 0.25
  )
  receptor_block <- shared_block * stats::rlnorm(
    length(shared_block), meanlog = 0, sdlog = 0.25
  )
  ligand <- 2.5 * ligand_block[blocks$block_code] *
    stats::rgamma(nrow(coordinates), shape = 7, rate = 7)
  receptor <- 3 * receptor_block[blocks$block_code] *
    stats::rgamma(nrow(coordinates), shape = 8, rate = 8)
...
extract_target <- function(expression, permutation_seed) {
  ...
  data.frame(
    ...
    analytic_log_sd = analytic_target$analytic_log_sd,
    influence_correlation = analytic_target$influence_correlation,
    inference_mode = analytic_target$inference_mode,
    fallback_reason = analytic_target$fallback_reason,
...
run_one <- function(replicate_index) {
  ...
  data.frame(
    ...
    null_log_sd = null$analytic_log_sd,
    null_influence_correlation = null$influence_correlation,
    ...
    alternative_log_sd = alternative$analytic_log_sd,
    alternative_influence_correlation = alternative$influence_correlation,
...
message(sprintf(
  "Running %d stratified analytic/permutation replicates with %d permutations...",
  n_replicates, nperm
))
...
replicates <- lapply(seq_len(n_replicates), function(index) {
  if (index == 1L || index == n_replicates || index %% 10L == 0L) {
    message(sprintf("Stratified calibration replicate %d/%d", index, n_replicates))
  }
  run_one(index)
})
...
summary <- data.frame(
  ...
  analytic_completion_rate = mean(
    replicates$null_mode == "experimental_stratified_analytic" &
      replicates$alternative_mode == "experimental_stratified_analytic"
  ),
  ...
  median_abs_null_influence_correlation = stats::median(abs(
    replicates$null_influence_correlation
  )),
